...
async function addCourse(req, res) {
  try {
    ...
    const reqBody = z.object({
      ...
      price: z.preprocess(
        (val) => Number(val),
        z.number().positive("price must be a positive")
      ),
    });
    const safeParse = reqBody.safeParse(req.body);
    if (!safeParse.success || !/^\d+$/.test(safeParse.data.price)) {
      await deleteFile(req.imgId);
      return res.status(400).json({
        message: safeParse.error.errors[0].message || "Invalid price value",
      });
    }
